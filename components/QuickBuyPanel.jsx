...
export default function QuickBuyPanel({
  ...
  const { name, category, options } = product;
  const price = activePrice ?? product.priceUSD;
  ...
  const handleConfirm = async () => {
    ...
    setConfirmedItem({ ...product, priceUSD: price, image: activeImage, selectedOptions, quantity: qty });

    const items = [{ ...product, priceUSD: price, selectedOptions, quantity: qty }];
          ...
          <>
            <div className="quickbuy-product-preview">
              ...
              <div className="quickbuy-product-info">
                ...
                <span className="quickbuy-product-price">${price.toFixed(2)}</span>
                <div className="quickbuy-inline-qty">
                  <button onClick={() => onQtyChange?.(Math.max(1, qty - 1))} aria-label="Disminuir" disabled={qty <= 1}>
                    <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3" strokeLinecap="round">
                      <line x1="5" y1="12" x2="19" y2="12" />
                    </svg>
                  </button>
                  <span>{qty}</span>
                  <button onClick={() => onQtyChange?.(Math.min(maxQty, qty + 1))} aria-label="Aumentar" disabled={qty >= maxQty}>
                    <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3" strokeLinecap="round">
                      ...
                      {values.map((val) => {
                        ...
                        return (
                          <button
                            ...
                            onClick={() => onOptionChange?.(optionKey, val.name)}
                          >
                            {val.name}
            ...
            <div className="quickbuy-body">
              ...
              <div className="quickbuy-form">
                <div className="qfield">
                  ...
                  <input className={`qinput${submitted && errors.name ? " error" : ""}`} type="text" placeholder="Escribe tu nombre completo" value={customer.name} onChange={(e) => update("name", e.target.value)} />
                  {submitted && errors.name && <span className="qerror">{errors.name}</span>}
                </div>
                <div className="qfield">
                  <label className="qlabel">Teléfono *</label>
                  <input className={`qinput${submitted && errors.phone ? " error" : ""}`} type="tel" placeholder="Ej. +53 5 123 4567" value={customer.phone} onChange={(e) => update("phone", e.target.value)} />
                    ...
                    <div className="qfield">
                      ...
                      {mode === "both" ? (
                        <>
                          ...
                          {isDelivery ? (
                            <>
                              <input className={`qinput${submitted && errors.address ? " error" : ""}`} type="text" placeholder="Dirección, ciudad, código postal" value={customer.address} onChange={(e) => update("address", e.target.value)} />
                ...
                <div className="qfield">
                  ...
                  {customer.payment === "Otro" && (
                    <>
                      <input className={`qinput${submitted && errors.paymentOther ? " error" : ""}`} type="text" placeholder="Describe tu método de pago" value={customer.paymentOther} onChange={(e) => update("paymentOther", e.target.value)} style={{ marginTop: "0.5rem" }} />
