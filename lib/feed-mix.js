...
// correcto — antes el "infinito" era en realidad un ciclo del catálogo.

export const PRODUCTS_PER_BUSINESS = 15;

// Orden por tendencia (cold start, UI_UX.md §7): ventas reales (peso fuerte)
...
export function rankByRecency(products, seed = 0) {
  ...
  return list
    .map((p, i) => {
      const base = times[i];
      // Sin fecha conocida cae al fondo: antes lo último que no se sabe
      // cuándo apareció que lo último que sí.
 ...
 *   2. Los productos que sobraron (ya no entran en un bloque completo) y los
 *      bisnes que no_publiers lugar en un bloque.
...
export function buildMixedFeed({
  ...
  while (items.length < count) {
    ...
    // Se acabaron los productos o los bisnes: no insistir, la fase 2
    // recoge lo que falte.
